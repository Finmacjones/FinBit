...
// Signal handling lives in the standalone wrapper (main.cpp); the embeddable
// loop below stops via the `stop` flag passed to run_relay().
...
int fb::server::run_relay(const RelayConfig& cfg, std::atomic<bool>& stop) {
    // Config → locals; the event-loop body below uses these names verbatim.
    std::string   bind_host    = cfg.bind_host;
    std::uint16_t port         = cfg.port;
    std::uint16_t ws_port      = cfg.ws_port;
    std::uint16_t tls_port     = cfg.tls_port;
    std::uint16_t tls_raw_port = cfg.tls_raw_port;
    std::string   tls_cert     = cfg.tls_cert;
    ...
    if (bind_host == "0.0.0.0" && !cfg.quiet) {
