...
- **DMs** — Signal Double Ratchet over libsodium, AES-256-GCM (native) /
  ...
  below). 176 native tests + 9 web smokes + 10 end-to-end shell demos pass.
- **Channels** — Two interoperable group-crypto paths:
  ...
  - **MLS / RFC 9420** (opt-in) — vendored
    ...
    186 MLS-build tests pass including end-to-end alice/bob
...
cmake -S . -B build
cmake --build build -j
ctest --test-dir build --output-on-failure   # 176 native tests
```
...
ctest --test-dir build-mls --output-on-failure   # 186 native tests
```
...
```bash
ctest --test-dir build --output-on-failure       # 176 tests, default
ctest --test-dir build-mls --output-on-failure   # 186 tests, FB_FEATURE_MLS=ON
```
